...
I, Anudeep Talari, hereby declare that this submission, emu.cpp is entirely my own work,
and asm.cpp,asm.emu and other test files and claims are a part of CS210 lab
...
int trace(int PC, int poc, ofstream& trcfile)
{
	...
	while(true)
	{
		// to check infinite loop
		int old_pc = PC;

		// Int 32 for operand and opcode of operation
		int32_t operand, operation;

		// Instruction at particular PC
		int32_t instr = memory[PC];

		// Retrieve operand and opcode of operation
